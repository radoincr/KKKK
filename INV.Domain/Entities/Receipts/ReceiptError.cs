...
public static class ReceiptError
{
    ...
    {
        return Error.NotFound("ReceiptError.ReceiptNotFound", $"Receipt with ID {receiptId} was not found.");
    }

    public static Error InvalidReceiptStatus(ReceiptStatus status)
    {
        return Error.Conflict("ReceiptError.InvalidReceiptStatus",
            $"The receipt status {status} is invalid for this operation.");
    }

    public static Error ReceiptAlreadyValidated(Guid receiptId)
    {
        return Error.Conflict("ReceiptError.ReceiptAlreadyValidated",
            $"Receipt with ID {receiptId} has already been validated.");
    }

    public static Error ReceiptProductNotFound(Guid productId)
    {
        return Error.NotFound("ReceiptError.ReceiptProductNotFound",
            $"Receipt product with ID {productId} was not found.");
    }

    public static Error InvalidReceiptProductQuantity(int quantity)
    {
        return Error.Problem("ReceiptError.InvalidReceiptProductQuantity",
            $"The quantity {quantity} is invalid for the receipt product.");
    }
}