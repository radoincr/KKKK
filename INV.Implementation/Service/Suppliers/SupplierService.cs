...
public class SupplierService : ISupplierService
{
    ...
    public async Task<Result> AddSupplier(Supplier supplier)
    {
        try
        {
            var errorList = await validateSupplierCreate(supplier);
    ...
    {
        var result = await supplierstorage.SelectAllSupplier();
    ...
    public async Task<List<SupplierInfo>> GetSupplierByName(string name)
    {
        ...
        var suppliers = await supplierstorage.SelectAllSupplier();
    ...
    private async Task<List<Error>> validateSupplierCreate(Supplier supplier)
    {
        ...
        var rcExists = await supplierstorage.SupplierExistsByRC(supplier.RC);
        if (rcExists)
            errors.Add(SupplierError.RCExsist(supplier.RC));

        var nisExists = await supplierstorage.SupplierExistsByNIS(supplier.NIS);
        if (nisExists)
            errors.Add(SupplierError.NISExsist);
        var ribExists = await supplierstorage.SupplierExistsByRIB(supplier.RIB);
        if (ribExists)
